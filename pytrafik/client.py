...
class Client:
    ...
    def get_nearby_stops(self, lat, long, query_params=None):
        data = self.get('/location.nearbystops?originCoordLat=' +
                        str(lat) + '&originCoordLong=' + str(long))
        return data['LocationList']['StopLocation']
    ...
    def get_arrivals(self, stopID, date=None, time=None, query_params=None):
        if date is not None and time is not None:
            data = self.get('/arrivalBoard?id=' + str(stopID) +
    ...
    def get_departures(self, stopID, date=None, time=None, query_params=None):
        if date is not None and time is not None:
            data = self.get('/departureBoard?id=' + str(stopID) +
                            '&date=' + date + '&time=' + time, query_params)
        else:
    ...
    def get_livemap(self, longitudeMax, latitudeMax, longitudeMin, latitudeMin, onlyRealtime='yes', query_params=None):
        ...
        data = self.get('/livemap?maxx=' + self.latlon_to_string_representation(longitudeMax) + '&maxy=' + self.latlon_to_string_representation(latitudeMax) +
                        '&minx=' + self.latlon_to_string_representation(longitudeMin) + '&miny=' + self.latlon_to_string_representation(latitudeMin), query_params)
